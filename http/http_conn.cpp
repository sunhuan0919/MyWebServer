...
http_conn::HTTP_CODE http_conn::process_read()
{
    ...
    while ((m_check_state == CHECK_STATE_CONTENT && line_status == LINE_OK) || ((line_status = parse_line()) == LINE_OK))
    {
        ...
        switch (m_check_state)
        {
            ...
            {//解析请求头
                ret = parse_headers(text); //解析请求头的一行，如果解析到空行且content_length不为0，则转到CHECK_STATE_CONTENT
                if (ret == BAD_REQUEST)
                    return BAD_REQUEST;
...
bool http_conn::write()
{
    ...
    while (1)
    {
        ...
        if (temp < 0)
        {
            ...
            {   //更新iovec结构体的指针和长度，并注册写事件，等待下一次写事件触发（当写缓冲区从不可写变为可写，触发epollout），
                //因此在此期间无法立即接收到同一用户的下一请求，但可以保证连接的完整性。
                modfd(m_epollfd, m_sockfd, EPOLLOUT, m_TRIGMode);
                return true;
            }
            unmap();//若不是因为缓冲区满了而失败，取消mmap映射，关闭连接
            return false;
        }
        //更新已发送字节
        ...
        {//若响应报文整体发送成功,则取消mmap映射,并判断是否是长连接.
            ...
            {
                //重新初始化HTTP对象**
                init();//长连接重置http类实例，注册读事件，不关闭连接，
                return true;
            }
            else
            {
                return false;//短连接直接关闭连接
            }
        }
    }
}
