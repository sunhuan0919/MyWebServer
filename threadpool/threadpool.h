...
void threadpool<T>::run()
{
    while (true)
    {
        ...
        if (1 == m_actor_model)
        {
            ...
            {
                if (request->read_once())//read_once()函数LT模式下只read一次，ET模式下会循环调用read将数据读完
                {
                    request->improv = 1;//improv置为1说明数据已从缓冲区读出，工作线程已开始process()
                    connectionRAII mysqlcon(&request->mysql, m_connPool);
                    request->process();//process先process_read，再process_write，然后注册写事件
                }
                else
                {
                    request->improv = 1;
